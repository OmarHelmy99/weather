...
for(let i=0 ; i<forcastData.length ; i++){
  ...
  console.log(forcastData[i+1].date);
...
if(navigator.geolocation){
  navigator.geolocation.getCurrentPosition(currentpoistion)

}else{
  console.log("no");
}
function currentpoistion(poistion){
  console.log(poistion);
 api(poistion.coords.latitude+","+poistion.coords.longitude);
}
